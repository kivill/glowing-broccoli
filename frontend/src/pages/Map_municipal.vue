  ...
  <q-page>
    <div style="position: absolute; z-index: 1000; right: 20px; top: 10px">
      ...
      <span v-if="isLoggedIn" class="q-gutter-x-xs">
        ...
        <span v-if="hasPermisson('municipality')">
          <q-btn
            push
            size="sm"
            :color="canEdit ? 'primary' : 'white'"
            :text-color="canEdit ? 'white' : 'primary'"
            label="Добавить план нового объекта"
            @click="setEdit(!canEdit)"
          />
        </span>
        <q-btn
    ...
    <div style="height: 100vh; width: 100%">
      <l-map ref="map" :zoom="zoom" :center="coords">
        ...
        <l-polygon
          ...
          fillColor="#41b782"
        >
          <l-popup>Предложение о строительстве нового парка</l-popup>
        </l-polygon>
        ...
        >
          ...
          ></l-icon>
          <l-popup>
            <q-btn dense color="green" size="sm" @click="savePoint">
              Одобрить
            </q-btn>
            <q-btn dense color="red" size="sm" @click="savePoint">
              Отклонить
            </q-btn>
          </l-popup>
        </l-marker>
